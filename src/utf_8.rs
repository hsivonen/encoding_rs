...
impl Utf8Encoder {
    ...
    pub fn encode_from_utf16_raw(&mut self,
        ...
        return encode_from_utf16_impl(src, dst);
    }
...
cfg_if! {
    if #[cfg(all(feature = "simd-accel", target_feature = "sse2"))] {
        ...
        #[inline(always)]
        fn encode_from_utf16_impl(src: &[u16], dst: &mut [u8])
                                  -> (EncoderResult, usize, usize) {
            let mut read = 0;
            let mut written = 0;
            'outer: loop {
                let read_plus_stride = read + STRIDE_SIZE;
                if read_plus_stride > src.len() || written + STRIDE_SIZE > dst.len() {
                    break;
                }
                ...
                if !(contains_surrogates(first) || contains_surrogates(second)) {
// No surrogates case
                    while read < read_plus_stride {
